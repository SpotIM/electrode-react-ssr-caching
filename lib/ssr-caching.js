...
let defaultGenCacheKey = props => JSON.stringify(saveProps);
...
ReactCompositeComponent.mountComponentCache = function mountComponentCache(transaction, hostParent, hostContainerInfo, context, parentDebugID) { // eslint-disable-line
  ...
  if (doCache) {
    ...
    if (strategy === "simple") {
      cacheType = "cache";
      template = {
        cacheKey: opts.genCacheKey ? opts.genCacheKey(saveProps, context) : defaultGenCacheKey(saveProps, context)
      };
...
exports.setDefaultGenCacheKey = function(fn) {
  defaultGenCacheKey = fn;
};
